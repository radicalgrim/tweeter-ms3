...
//
//    @BeforeEach
//    void setup() { storyDAOSpy = Mockito.spy(new StoryDAO()); }
//
//    @Test
//    void testGetStory_noStatusesForUser() {
//        List<Status> statuses = Collections.emptyList();
//        Mockito.when(storyDAOSpy.getDummyStory()).thenReturn(statuses);
//
//        StoryRequest request = new StoryRequest(status1.getUser().getAlias(), 10, null);
...
//
//    @Test
//    void testGetStory_oneStatusForUser_limitGreaterThanStatuses() {
...
//        Mockito.when(storyDAOSpy.getDummyStory()).thenReturn(statuses);
//
//        StoryRequest request = new StoryRequest(status3.getUser().getAlias(), 2, null);
//        StoryResponse response = storyDAOSpy.getStory(request);
//
...
//        Mockito.when(storyDAOSpy.getDummyStory()).thenReturn(statuses);
//
//        StoryRequest request = new StoryRequest(status5.getUser().getAlias(), 2, null);
//        StoryResponse response = storyDAOSpy.getStory(request);
//
//        // Verify first page
//        Assertions.assertEquals(2, response.getStatuses().size());
...
//        request = new StoryRequest(status5.getUser().getAlias(), 2, response.getStatuses().get(1).getTimestamp());
...
//        Mockito.when(storyDAOSpy.getDummyStory()).thenReturn(statuses);
//
//        StoryRequest request = new StoryRequest(status6.getUser().getAlias(), 2, null);
//        StoryResponse response = storyDAOSpy.getStory(request);
//
//        // Verify first page
//        Assertions.assertEquals(2, response.getStatuses().size());
...
//
//        // Get and verify second page
//        request = new StoryRequest(status6.getUser().getAlias(), 2, response.getStatuses().get(1).getTimestamp());
//        response = storyDAOSpy.getStory(request);
//
//        Assertions.assertEquals(2, response.getStatuses().size());
//        Assertions.assertTrue(response.getStatuses().contains(status4));
